"""FastESM 3-pane layout: top bar · left nav · center · right AI rail.

Server-rendered FastHTML + HTMX; one block of vanilla JS for the streaming chat.
The left nav is **role-aware** (RBAC): Employee / Agent / Manager / Admin each
see a different set of screens.
"""
...
# FastESM palette — enterprise indigo, amber warn, rose breach, emerald ok.
LAYOUT_CSS = """
:root{
  --bg:#f4f5fb; --surface:#ffffff; --surface-2:#eceef8; --border:#dfe1f0; --text:#1a1c2e;
  --text-dim:#4b4f6b; --text-mute:#868aa6; --accent:#4f46e5; --accent-hover:#4338ca;
  --accent-light:#e0e7ff; --ok:#059669; --ok-light:#d1fae5; --warn:#d97706; --warn-light:#fef3c7;
...
.app{display:grid;grid-template-columns:236px 1fr var(--rail,340px);grid-template-rows:52px 1fr;
...
.role-switch{display:flex;gap:4px;background:var(--surface-2);padding:3px;border-radius:8px;}
.role-switch a{padding:4px 10px;border-radius:6px;font-size:12px;font-weight:600;color:var(--text-mute);}
.role-switch a.active{background:var(--accent);color:#fff;} .role-switch a:hover{text-decoration:none;color:var(--accent);}
.role-switch a.active:hover{color:#fff;}
...
.page-title{display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;gap:16px;}
...
.pill.submitted{background:#dbeafe;color:#1d4ed8;} .pill.pendingapproval{background:var(--warn-light);color:#92400e;}
.pill.approved{background:var(--accent-light);color:var(--accent-hover);} .pill.inprogress{background:#cffafe;color:#0e7490;}
.pill.fulfilled{background:var(--ok-light);color:#065f46;} .pill.rejected{background:var(--breach-light);color:#9f1239;} .pill.cancelled{background:#e2e8f0;color:#475569;}
...
.pill.ok2{background:var(--ok-light);color:#065f46;} .pill.neutral{background:#e2e8f0;color:#475569;}
.sla{display:inline-block;padding:2px 9px;border-radius:6px;font-size:11px;font-weight:600;white-space:nowrap;}
.sla.ok{background:var(--ok-light);color:#065f46;} .sla.warn{background:var(--warn-light);color:#92400e;} .sla.breach{background:var(--breach-light);color:#9f1239;} .sla.neutral{background:var(--surface-2);color:var(--text-mute);}
.funnel-row{display:grid;grid-template-columns:150px 1fr 70px;align-items:center;gap:10px;margin-bottom:7px;font-size:13px;}
...
.bubble.requester{align-self:flex-start;background:var(--surface);}
...
.btn.sm{padding:4px 9px;font-size:12px;} .btn.ok{border-color:var(--ok);color:var(--ok);} .btn.ok:hover{background:var(--ok);color:#fff;}
...
.mini-select{padding:3px 7px;border:1px solid var(--border);border-radius:6px;font-size:12px;background:var(--surface);max-width:180px;}
...
.catalog-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px;margin-bottom:8px;}
.svc-card{background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:16px 18px;display:flex;flex-direction:column;gap:8px;transition:box-shadow .15s,border-color .15s;}
.svc-card:hover{border-color:var(--accent);box-shadow:0 6px 18px rgba(79,70,229,.10);}
.svc-icon{font-size:26px;} .svc-card h4{margin:0;font-size:15px;} .svc-card .desc{color:var(--text-dim);font-size:12.5px;line-height:1.45;flex:1;}
.svc-meta{display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-top:2px;}
.svc-tag{font-size:11px;color:var(--text-mute);background:var(--surface-2);border-radius:999px;padding:2px 8px;}
.form-field{margin-bottom:12px;} .form-field label{display:block;font-size:12.5px;font-weight:600;color:var(--text-dim);margin-bottom:4px;}
.form-field .req{color:var(--breach);} .form-field input,.form-field select,.form-field textarea{width:100%;padding:9px 11px;border:1px solid var(--border);border-radius:8px;font-size:13px;font-family:inherit;background:var(--surface);}
.form-field textarea{min-height:80px;resize:vertical;} .form-field input:focus,.form-field select:focus,.form-field textarea:focus{border-color:var(--accent);outline:none;box-shadow:0 0 0 3px var(--accent-light);}
.form-field .hint{font-size:11px;color:var(--text-mute);margin-top:3px;}
.login-wrap{height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#eef0fb 0%,#e0e7ff 100%);}
...
.role-pick-wrap{height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,#eef0fb 0%,#e0e7ff 100%);gap:18px;}
.role-pick-grid{display:grid;grid-template-columns:repeat(4,minmax(150px,180px));gap:16px;}
.role-tile{background:#fff;border:1px solid var(--border);border-radius:14px;padding:22px 18px;text-align:center;cursor:pointer;transition:box-shadow .15s,border-color .15s,transform .12s;color:var(--text);}
.role-tile:hover{border-color:var(--accent);box-shadow:0 12px 28px rgba(79,70,229,.16);transform:translateY(-2px);text-decoration:none;}
.role-tile .ic{font-size:34px;} .role-tile .rn{font-weight:700;margin-top:8px;font-size:16px;} .role-tile .rd{color:var(--text-mute);font-size:12px;margin-top:4px;line-height:1.4;}
...
# Nav items keyed by role — RBAC-driven views.
NAV_BY_ROLE = {
    "Employee": [
        ("SELF-SERVICE", [("catalog", "Service Catalog", "🛍️", "/catalog"),
                          ("myrequests", "My Requests", "📦", "/my-requests")]),
        ("KNOWLEDGE", [("kb", "Knowledge Base", "📚", "/kb")]),
        ("ASSISTANT", [("ai", "AI Assistant", "🤖", "/ai")]),
    ],
    "Agent": [
        ("OVERVIEW", [("dashboard", "Dashboard", "📊", "/"), ("ai", "AI Assistant", "🤖", "/ai")]),
        ("FULFILMENT", [("requests", "Request Queue", "🎫", "/requests"),
                        ("catalog", "Service Catalog", "🛍️", "/catalog")]),
        ("KNOWLEDGE", [("kb", "Knowledge Base", "📚", "/kb")]),
    ],
    "Manager": [
        ("OVERVIEW", [("dashboard", "Dashboard", "📊", "/"), ("ai", "AI Assistant", "🤖", "/ai")]),
        ("WORKFLOW", [("approvals", "Approvals", "✅", "/approvals"),
                      ("requests", "Request Queue", "🎫", "/requests")]),
        ("KNOWLEDGE", [("kb", "Knowledge Base", "📚", "/kb")]),
    ],
    "Admin": [
        ("OVERVIEW", [("dashboard", "Dashboard", "📊", "/"), ("ai", "AI Assistant", "🤖", "/ai")]),
        ("SERVICE MGMT", [("requests", "Request Queue", "🎫", "/requests"),
                          ("approvals", "Approvals", "✅", "/approvals"),
                          ("catalog", "Service Catalog", "🛍️", "/catalog")]),
        ("CONFIGURE", [("designer", "Form & Workflow Designer", "🛠️", "/designer"),
                       ("people", "People & Departments", "🏢", "/people")]),
        ("KNOWLEDGE", [("kb", "Knowledge Base", "📚", "/kb"),
...
SAMPLE_QUESTIONS = [
    ...
    "Which department has the biggest backlog?",
]


def role_switch(active_role):
    from db import ROLES
    return Div(*[A(r, href=f"/role/{r}", cls="active" if r == active_role else "") for r in ROLES],
              cls="role-switch")


def topbar(env, user_email, role):
    right = Div(
        role_switch(role) if user_email else None,
        Button(NotStr("&laquo; Chat"), id="copilot-topbar-toggle", cls="btn",
               onclick="toggleCopilot()") if user_email else None,
    ...
    return Div(Div(Span(cls="brand-dot"), Span("Fast", style="font-weight:800;"),
                   Span("ESM", style="color:var(--accent);font-weight:700;letter-spacing:.5px;"), cls="brand"),
               right, cls="topbar")


def left_pane(active, role):
    sections = []
    for name, items in NAV_BY_ROLE.get(role, NAV_BY_ROLE["Admin"]):
...
def right_pane_chat(thread_id):
    return Div(
        Div(H3("AI Assistant"),
            ...
            cls="right-header"),
        Div(Div(P("Ask about requests, SLAs, approvals or the service catalog — or tap a question below.",
            ...
            Form(Input(type="hidden", name="thread_id", value=thread_id, id="thread-id"),
                 Div(Input(type="text", name="message", id="chat-input",
                           placeholder="Ask a question or /sla /requests /help …", autocomplete="off"),
...
def page(active, env, user_email, role, thread_id, *content, right_override=None, extra_head=None):
    ...
    head = [Title("FastESM"),
            ...
            Style(LAYOUT_CSS)]
    if extra_head:
        head.extend(extra_head if isinstance(extra_head, (list, tuple)) else [extra_head])
    return (*head,
            Div(topbar(env, user_email, role), left_pane(active, role),
...
def kpi_card(label, value, trend="", tone=""):
    return Div(Div(label, cls="label"),
               Div(f"{value:,}" if isinstance(value, (int, float)) and not isinstance(value, bool) else str(value),
                   cls="value"),
...
function toggleCopilot(){var app=document.querySelector('.app');if(!app)return;app.classList.toggle('right-collapsed');
  ...
  try{localStorage.setItem('esmCollapsed',app.classList.contains('right-collapsed')?'1':'0');}catch(e){}_sync();}
function toggleExpand(){var app=document.querySelector('.app');if(!app)return;app.classList.remove('right-collapsed');app.classList.toggle('right-expanded');
  try{localStorage.setItem('esmExpanded',app.classList.contains('right-expanded')?'1':'0');localStorage.setItem('esmCollapsed','0');}catch(e){}_sync();}
(function(){try{var app=document.querySelector('.app');if(!app)return;
  if(localStorage.getItem('esmCollapsed')==='1')app.classList.add('right-collapsed');
  else if(localStorage.getItem('esmExpanded')==='1')app.classList.add('right-expanded');}catch(e){}})();
